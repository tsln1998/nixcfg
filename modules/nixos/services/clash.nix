...
in
{
  options.services.clash = {
    enable = lib.mkEnableOption "Clash, A rule-based proxy in Go";
    ...
    tunMode = lib.mkEnableOption ''
      necessary permission for Cihomo's systemd service for TUN mode to function properly.

      Keep in mind, that you still need to enable TUN mode manually in Cihomo's configuration
    '';

    tproxyMode = lib.mkEnableOption ''
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services."clash" = {
      ...
      serviceConfig =
        ...
        // lib.optionalAttrs cfg.tproxyMode {
